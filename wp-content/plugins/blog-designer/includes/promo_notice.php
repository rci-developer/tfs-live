...
if (!function_exists('bd_load_plugin')) {
    ...
    function bd_load_plugin() {
        ...
        $blogDesigner['promo_time_review'] = get_option('blog_designer_promo_time_review');
        ...
        if (empty($blogDesigner['promo_time_review'])) {
            $blogDesigner['promo_time_review'] = time();
            update_option('blog_designer_promo_time_review', $blogDesigner['promo_time_review']);
        }


        if ($blogDesigner['promo_time_review'] < (time() - (60 * 60 * 24 * 3))) {
            update_option('blog_designer_promo_time', $blogDesigner['promo_time_review']);
        }

        // Are we to show the Blog Designer promo
        if (!empty($blogDesigner['promo_time']) && $blogDesigner['promo_time'] > 0) {
...
if (!function_exists('blog_designer_promo')) {
    ...
    function blog_designer_promo() {
        ...
        $blogDesigner['promo_time'] = get_option('blog_designer_promo_time');
        if ($blogDesigner['promo_time'] < (time() - (60 * 60 * 24 * 3))) {
