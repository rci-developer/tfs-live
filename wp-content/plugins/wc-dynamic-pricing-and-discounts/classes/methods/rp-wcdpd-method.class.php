...
abstract class RP_WCDPD_Method extends RP_WCDPD_Item
{
    ...
    public function get_adjustments($rule, $cart_items = null)
    {
        ...
        if (RP_WCDPD_Conditions::rule_conditions_are_matched($rule, array('context' => $this->context, 'cart_items' => $cart_items))) {
    ...
    public function get_reference_amount($adjustment, $base_amount = null)
    {
        ...
        if (in_array($selection_method, array('bigger_discount', 'smaller_discount', 'bigger_fee', 'smaller_fee'), true)) {
            $reference_amount = (float) RP_WCDPD_Pricing::get_adjustment_value($adjustment['rule']['pricing_method'], $adjustment['rule']['pricing_value'], $base_amount);
