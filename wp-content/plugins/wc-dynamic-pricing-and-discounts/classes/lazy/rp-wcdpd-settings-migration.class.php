            ...
            if (!empty($options) && !empty($options[1]) && !$already_migrated) {
                ...
                if (!empty($options['pricing']['sets']) && is_array($options['pricing']['sets'])) {
                    foreach ($options['pricing']['sets'] as $rule) {
                        ...
                        // UID
                        $current_rule['uid'] = 'rp_wcdpd_' . RightPress_Helper::get_hash();
                        ...
                        if (!empty($rule['valid_from'])) {
                            ...
                            $condition_date = RightPress_Helper::is_date((string) $rule['valid_from'], 'Y-m-d') ? (string) $rule['valid_from'] : '2100-01-01';

                            $current_rule['conditions'][] = array(
                                'uid'           => 'rp_wcdpd_' . RightPress_Helper::get_hash(),
                                'type'          => 'time__date',
                                'method_option' => 'from',
                                'date'          => $condition_date,
                            );
                        ...
                        if (!empty($rule['valid_until'])) {
                            ...
                            $condition_date = RightPress_Helper::is_date((string) $rule['valid_until'], 'Y-m-d') ? (string) $rule['valid_until'] : '1999-12-31';
                ...
                if (!empty($options['discounts']['sets']) && is_array($options['discounts']['sets'])) {
                    foreach ($options['discounts']['sets'] as $cart_discount) {
                        ...
                        if (!empty($cart_discount['valid_from'])) {
                            ...
                            // If date is not valid, add date far in the future to block condition
                            $condition_date = RightPress_Helper::is_date((string) $cart_discount['valid_from'], 'Y-m-d') ? (string) $cart_discount['valid_from'] : '2100-01-01';
                        ...
                        if (!empty($cart_discount['valid_until'])) {
                            ...
                            // If date is not valid, add date far in the past to block condition
                            $condition_date = RightPress_Helper::is_date((string) $cart_discount['valid_until'], 'Y-m-d') ? (string) $cart_discount['valid_until'] : '1999-12-31';
