	...
	do_action( 'woothemes_testimonials_before', $args );
		...
		if ( ! is_wp_error( $query ) && is_array( $query ) && count( $query ) > 0 ) {
			...
			if ( $args['pagination'] == true && count( $query ) > 1 && $args['effect'] != 'none' ) {
				...
				$html .= '<a href="#" class="btn-prev">' . apply_filters( 'woothemes_testimonials_prev_btn', '&larr; ' . esc_html__( 'Previous', 'royal' ) ) . '</a>' . "\n";
		        $html .= '<a href="#" class="btn-next">' . apply_filters( 'woothemes_testimonials_next_btn', esc_html__( 'Next', 'royal' ) . ' &rarr;' ) . '</a>' . "\n";
		        $html .= '</div><!--/.pagination-->' . "\n";
			}
