...
class Woothemes_Testimonials_Taxonomy {
	...
	public function __construct ( $token = 'testimonial-category', $singular = '', $plural = '', $args = array() ) {
		...
		if ( '' == $this->singular ) $this->singular = esc_html__( 'Category', 'royal' );
		if ( '' == $this->plural ) $this->plural = esc_html__( 'Categories', 'royal' );
	...
	private function _get_default_labels () {
		return array(
			    'name'                => sprintf( _x( '%s', 'taxonomy general name', 'royal' ), $this->plural ),
			    'singular_name'       => sprintf( _x( '%s', 'taxonomy singular name', 'royal' ), $this->singular ),
			    'search_items'        => sprintf( esc_html__( 'Search %s', 'royal' ), $this->plural ),
			    'all_items'           => sprintf( esc_html__( 'All %s', 'royal' ), $this->plural ),
			    'parent_item'         => sprintf( esc_html__( 'Parent %s', 'royal' ), $this->singular ),
			    'parent_item_colon'   => sprintf( esc_html__( 'Parent %s:', 'royal' ), $this->singular ),
			    'edit_item'           => sprintf( esc_html__( 'Edit %s', 'royal' ), $this->singular ),
			    'update_item'         => sprintf( esc_html__( 'Update %s', 'royal' ), $this->singular ),
			    'add_new_item'        => sprintf( esc_html__( 'Add New %s', 'royal' ), $this->singular ),
			    'new_item_name'       => sprintf( esc_html__( 'New %s Name', 'royal' ), $this->singular ),
			    'menu_name'           => sprintf( esc_html__( '%s', 'royal' ), $this->plural )
			  );
