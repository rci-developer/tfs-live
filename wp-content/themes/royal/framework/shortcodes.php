    ...
    ?>
        ...
        <form action="<?php the_permalink(); ?>" method="get" id="contact-form" class="contact-form <?php echo $class; ?>">
            ...
            <div class="form-group">
              <p class="form-name">
                <label for="name" class="control-label"><?php esc_html_e('Name and Surname', 'royal') ?> <span class="required">*</span></label>
            ...
            <div class="form-group">
                <p class="form-name">
                  <label for="contact-email" class="control-label"><?php esc_html_e('Email', 'royal') ?> <span class="required">*</span></label>
            ...
            <div class="form-group">
              <p class="form-name">
                <label for="contact-website" class="control-label"><?php esc_html_e('Website', 'royal') ?></label>
            ...
            <div class="form-group">
              <p class="form-textarea">
                <label for="contact_msg" class="control-label"><?php esc_html_e('Message', 'royal'); ?> <span class="required">*</span></label>
            ...
            <p class="pull-right">
              ...
              <span class="spinner"><?php esc_html_e('Sending...', 'royal') ?></span>
              <button class="btn btn-black big" id="submit" type="submit"><?php esc_html_e('Send message', 'royal') ?></button>
            </p>
	...
	function et_brands($atts) {
		...
		if ($count > 0) {
		    ...
		    foreach ($terms as $term) {
				...
				if($thumbnail_id) {
					$output .= '<a href="' . get_term_link( $term ) . '" title="' . esc_html__('View all products from ', 'royal') . $term->name . '"><img src="' . etheme_get_image($thumbnail_id) . '" title="' . $term->name . '"/></a>';
				} else {
					$output .= '<h3><a href="' . get_term_link( $term ) . '" title="' . esc_html__('View all products from ', 'royal') . $term->name . '">' . $term->name . '</a></h3>';
				}
				$output .= '</div>';
		    }
	...
	function etheme_search($atts) {
		...
		$output .= '<div class="et-mega-search '.$class.'" data-products="'.$products.'" data-count="'.$count.'" data-posts="'.$posts.'" data-portfolio="'.$portfolio.'" data-pages="'.$pages.'" data-testimonial="'.$testimonial.'" data-images="'.$images.'">';
			...
			$output .= '<form method="get" action="'.home_url( '/' ).'">';
				$output .= '<input type="text" value="'.$search_input.'" name="s" autocomplete="off" placeholder="'.esc_html__('Search', 'royal').'"/>';
...
if(!function_exists('et_twitter_slider')) {
	function et_twitter_slider($atts) {
		...
		if(empty($consumer_key) || empty($consumer_secret) || empty($user_token) || empty($user_secret) || empty($user)) {
			return esc_html__('Not enough information', 'royal');
		}
...
function etheme_featured_shortcodes($atts, $content=null){
    ...
    extract(shortcode_atts(array(
        ...
        'limit' => 50,
        'categories' => '',
        'title' => esc_html__('Featured Products', 'royal')
    ), $atts));
...
function etheme_new_shortcodes($atts, $content=null){
    ...
    extract(shortcode_atts(array(
        ...
        'limit' => 50,
        'categories' => '',
        'title' => esc_html__('Latest Products', 'royal')
    ), $atts));
        ...
        <div class="project-navigation">
            <?php if(!empty($prev_post)) : ?>
                <div class="pull-left prev-project">
                    <a href="<?php echo get_permalink($prev_post->ID); ?>" class="btn border-grey btn-xmedium project-nav"><?php esc_html_e('Previous', 'royal'); ?></a>
                    <div class="hide-info">
            ...
            <?php if(!empty($next_post)) : ?>
                <div class="pull-right next-project">
                    <a href="<?php echo get_permalink($next_post->ID); ?>" class="btn border-grey btn-xmedium project-nav"><?php esc_html_e('Next', 'royal'); ?></a>
                    <div class="hide-info">
...
if(!function_exists('etheme_share_shortcode')) {
	function etheme_share_shortcode($atts, $content = null) {
	    ...
	    if($twitter == 1) {
	        $html .= '
	                <li>
	                    <a href="https://twitter.com/share?url='.$permalink.'&text='.$post_title.'" class="'.$tooltip_class.'" title="'.__('Twitter', 'royal').'" target="_blank">
	    ...
	    if($facebook == 1) {
	        $html .= '
	                <li>
	                    <a href="http://www.facebook.com/sharer.php?u='.$permalink.'" class="'.$tooltip_class.'" title="'.__('Facebook', 'royal').'" target="_blank">
	    ...
	    if($pinterest == 1) {
	        $html .= '
	                <li>
	                    <a href="http://pinterest.com/pin/create/button/?url='.$permalink.'&amp;media='.$image.'&amp;description='.$post_title.'" class="'.$tooltip_class.'" title="'.__('Pinterest', 'royal').'" target="_blank">
	    ...
	    if($google == 1) {
	        $html .= '
	                <li>
	                    <a href="http://plus.google.com/share?url='.$permalink.'&title='.$text.'" class="'.$tooltip_class.'" title="'.__('Google +', 'royal').'" target="_blank">
	    ...
	    if($mail == 1) {
	        $html .= '
	                <li>
	                    <a href="mailto:enteryour@addresshere.com?subject='.$post_title.'&amp;body=Check%20this%20out:%20'.$permalink.'" class="'.$tooltip_class.'" title="'.__('Mail to friend', 'royal').'" target="_blank">
...
function etheme_team_member_shortcode($atts, $content = null) {
    ...
    $html .= '<div class="team-member member-type-'.$a['type'].' '.$a['class'].'">';
        ...
        $html .= '<div class="member-details">';
            ...
            if($a['email'] != ''){
                $html .= '<p class="member-email"><span>'.__('Email:', 'royal').'</span> <a href="mailto:'.$a['email'].'">'.$a['email'].'</a></p>';
            }
...
function etheme_featured_post_shortcode($atts) {
        ...
        <?php while ( $the_query->have_posts() ) : $the_query->the_post();  $postId = get_the_ID(); ?>
            ...
            <div class="featured-posts <?php echo $a['class']; ?>">
                <?php if ($a['title'] != ''): ?>
                    ...
                    <?php if ($a['more_posts']): ?>
                            <?php echo '<a href="'.$posts_url.'" class="show-all-posts hidden-tablet hidden-phone">'.__('View more posts', 'royal').'</a>'; ?>
                    <?php endif ?>
                <?php endif ?>
                <div class="featured-post row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="post-description">
                            <?php the_excerpt(); ?>
                            <a href="<?php the_permalink(); ?>" class="button read-more"><?php esc_html_e('Read More', 'royal') ?></a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <?php else:  ?>
        ...
        <p><?php esc_html_e( 'Sorry, no posts matched your criteria.', 'royal' ); ?></p>
...
function et_promo_shortcode($atts) {
	...
	$output .= '<div class="popup_link ' . $rand . '' . esc_html( $class ) . '"><a class="etheme-popup open-click" href="#etheme-popup">' . esc_html__( 'Newsletter', 'royal' ) . '</a></div>';
