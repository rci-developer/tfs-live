...
if ( ! class_exists('Walker_Nav_Menu')) return; 
...
class Et_Navigation extends Walker_Nav_Menu {
    ...
    function start_el ( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( $depth == 1 && ( get_post_meta($item_id, '_menu-item-column_width', true ) != '') ) {
            $class_names .= ' custom-column';
        }
...
class Et_Navigation_Mobile extends Walker_Nav_Menu
{
    ...
    function start_el ( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $class_names = join( ' ', apply_filters( 'nav_menu_css_class', array_filter( $classes ), $item, $args ) );
        $class_names .= ($design != '' && $design == 'full-width open-by-click') ? ' open-by-click' : '';
        $class_names = ' class="' . esc_attr( $class_names ) . ' "';
