...
if ( $type != 'accordion' && etheme_get_option('single_product_layout') == 'fixed' && etheme_get_option('tabs_location') == 'after_image' ) {
    $type = 'accordion';
}
    ...
    <div class="tabs <?php echo esc_attr( $type ); ?>">
        <ul class="tabs-nav">
            <?php foreach ( $tabs as $key => $tab ) : $i++; ?>
                <li>
                    <a href="#tab_<?php echo $key ?>" id="tab_<?php echo $key ?>" class="tab-title <?php if($i == 1 && $first_tab[0] != 1) echo 'opened'; ?>"><?php echo apply_filters( 'woocommerce_product_' . $key . '_tab_title', $tab['title'], $key ) ?></a>
                </li>
                <?php if ( $type == 'accordion' ): ?>
                    <div class="tab-content tab-<?php echo $key ?>" id="content_tab_<?php echo $key ?>" <?php if($i == 1 && $first_tab[0] != 1) echo 'style="display:block;"'; ?>>
